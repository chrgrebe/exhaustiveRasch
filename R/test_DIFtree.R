test_DIFtree <- function(items=NULL, DIFvars=NULL, dset=NULL, na.rm=T, model=NULL, modelType=NULL, alpha=0.1){
  ...
  if (is.null(model)){
    ...
    try(suppressWarnings({
      ...
      items <- which(colnames(dset) %in% colnames(model$X))}), silent=T)
  }
  ...
  if (modelType=="RM"){
    try(suppressWarnings({DIF_tree <- psychotree::raschtree(rasch ~., data=ds_DIF)}), silent=T)
  }
  if (modelType=="RSM"){
    try(suppressWarnings({DIF_tree <- psychotree::rstree(rasch ~., data=ds_DIF)}), silent=T)
  }
  if (modelType=="PCM"){
    try(suppressWarnings({DIF_tree <- psychotree::pctree(rasch ~., data=ds_DIF)}), silent=T)
  }
