apply_combo_rules <- function(full, combo_length=3:length(full), forced_items=NULL, rules=NULL){
  ...
  check_combo_rules <- function(full, forced_items=NULL, rules=NULL){
    ...
    } else{passed_forced <-T} # if no rule defined: passed=T
    ...
    if (length(min_rules)>0){ # if no rule defined: passed=T
    ...
    } else{passed_min_rules <- T} # if no rule defined: passed=T
    ...
    } else{passed_forbidden_rules <- T} # if no rule defined: passed=T
