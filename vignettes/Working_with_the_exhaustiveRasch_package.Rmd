...
1.  we can make the decision based on an information criterion. Since we did not set the ICs argument to TRUE when calling the *exhaustive_tests* function, our *passed_exRA* object passed_ADL does not contain them (otherwise they would be available in the \@IC slot). We can add the information criteria later by using the *add_ICs()* function.

    ```{r information criteria (ADL example)}
    passed_rem <- add_ICs(passed_rem)
    print(passed_rem@IC)
    ```

    The choice for the final model could now fall on the one with the lowest value for your preferred information criterion.
